...
var nonInteractiveRoles = new Set(
  roleKeys.filter(function(name) {
...
var nonInteractiveElementRoleSchemas = elementRoleEntries.reduce(function(
  accumulator,
  _ref
) {
  ...
  if (
    [].concat(_toConsumableArray(roleSet)).every(function(role) {
...
var interactiveElementRoleSchemas = elementRoleEntries.reduce(function(
  accumulator,
  _ref3
) {
  ...
  if (
    [].concat(_toConsumableArray(roleSet)).some(function(role) {
...
var nonInteractiveAXObjects = new Set(
  []
    .concat(_toConsumableArray(_axobjectQuery.AXObjects.keys()))
    .filter(function(name) {
      return (0,
      _arrayIncludes2.default)(["window", "structure"], _axobjectQuery.AXObjects.get(name).type);
    })
);

var nonInteractiveElementAXObjectSchemas = []
  .concat(_toConsumableArray(_axobjectQuery.elementAXObjects))
  .reduce(function(accumulator, _ref5) {
    ...
    if (
      [].concat(_toConsumableArray(AXObjectSet)).every(function(role) {
...
function checkIsNonInteractiveElement(tagName, attributes) {
  function elementSchemaMatcher(elementSchema) {
    return (
      ...
      (0, _attributesComparator2.default)(elementSchema.attributes, attributes)
    );
  }
  // Check in elementRoles for inherent non-interactive role associations for
  // this element.
  var isInherentNonInteractiveElement = nonInteractiveElementRoleSchemas.some(
  ...
  // this element.
  var isInherentInteractiveElement = interactiveElementRoleSchemas.some(
    elementSchemaMatcher
  );
  ...
  var isNonInteractiveAXElement = nonInteractiveElementAXObjectSchemas.some(
    elementSchemaMatcher
  );
 ...
 */
var isNonInteractiveElement = function isNonInteractiveElement(
  tagName,
  attributes
) {
