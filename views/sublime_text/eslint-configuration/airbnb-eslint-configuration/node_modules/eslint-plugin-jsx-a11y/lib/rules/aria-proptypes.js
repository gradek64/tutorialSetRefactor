...
var errorMessage = function errorMessage(name, type, permittedValues) {
  switch (type) {
    case "tristate":
      return (
        "The value for " + name + ' must be a boolean or the string "mixed".'
      );
    case "token":
      return (
        "The value for " +
        name +
        " must be a single token from the following: " +
        permittedValues +
        "."
      );
    case "tokenlist":
      return (
        "The value for " +
        name +
        " must be a list of one or more tokens from the following: " +
        permittedValues +
        "."
      );
    case "boolean":
...
var validityCheck = function validityCheck(
  value,
  expectedType,
  permittedValues
) {
  switch (expectedType) {
    ...
    case "tokenlist":
      return (
        typeof value === "string" &&
        value.split(" ").every(function(token) {
    ...
    return {
      JSXAttribute: function JSXAttribute(attribute) {
        ...
        if (
          ...
          _ariaQuery.aria.get(normalizedName) === undefined
        ) {
          return;
        }
