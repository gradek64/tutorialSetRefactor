    ...
    function testInvalidTemplate(item) {
      ...
      if (typeof item.errors === "number") {
        ...
        for (let i = 0, l = item.errors.length; i < l; i++) {
          ...
          if (
            typeof item.errors[i] === "string" ||
            item.errors[i] instanceof RegExp
          ) {
            ...
            if (item.errors[i].type) {
              ...
              assert.equal(
                messages[i].nodeType,
                item.errors[i].type,
                `Error type should be ${item.errors[i].type}, found ${
                  messages[i].nodeType
                }`
              );
            }
            ...
            if (item.errors[i].hasOwnProperty("endColumn")) {
              // eslint-disable-next-line no-restricted-properties
              assert.equal(
                messages[i].endColumn,
                item.errors[i].endColumn,
                `Error endColumn should be ${item.errors[i].endColumn}`
              );
            }
          } else {
