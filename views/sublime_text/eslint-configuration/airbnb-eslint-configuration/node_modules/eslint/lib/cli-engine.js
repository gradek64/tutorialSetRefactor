...
function processText(
  ...
  const fixedResult = linter.verifyAndFix(text, config, {
    ...
    preprocess:
      processor && (rawText => processor.preprocess(rawText, filename)),
    postprocess:
      processor &&
      (problemLists => processor.postprocess(problemLists, filename))
  });
...
function createIgnoreResult(filePath, baseDir) {
  ...
  const isInNodeModules =
    baseDir && /^node_modules/.test(path.relative(baseDir, filePath));
  const isInBowerComponents =
    baseDir && /^bower_components/.test(path.relative(baseDir, filePath));
