  ...
  create: function(context) {
    ...
    function reportNoEndingNewline(node, token, spacing) {
      context.report({
        ...
        fix: function(fixer) {
          ...
          return fixer.replaceTextRange(
            [previousToken.range[1], token.range[0]],
            spacing === SPACING.always ? " " : ""
          );
        }
      });
    ...
    function reportNoBeginningSpace(node, token) {
      context.report({
        ...
        fix: function(fixer) {
          ...
          const rangeEndRef = leadingComments.length
            ? leadingComments[0]
            : nextToken;
    ...
    function reportNoEndingSpace(node, token) {
      context.report({
        ...
        fix: function(fixer) {
          const previousToken = sourceCode.getTokenBefore(token);
          const previousNode = sourceCode.getNodeByRangeIndex(
            previousToken.range[0]
          );
          const trailingComments = sourceCode.getComments(previousNode)
            .trailing;
          const rangeStartRef = trailingComments.length
