...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    const propWrapperFunctions = new Set(
      context.settings.propWrapperFunctions || []
    );
    ...
    function isPropTypesUsage(node) {
      ...
      const isStatelessFunctionUsage =
        node.object.name === "props" && !isAssignmentToProp(node);
      const isNextPropsUsage =
        node.object.name === "nextProps" && inComponentWillReceiveProps();
        ...
        switch (callName) {
          case "shape":
            ...
            iterateProperties(argument.properties, (childKey, childValue) => {
              shapeTypeDefinition.children[
                childKey
              ] = buildReactDeclarationTypes(childValue);
            });
      ...
      switch (annotation.type) {
        ...
        case "ObjectTypeAnnotation":
          ...
          iterateProperties(annotation.properties, (childKey, childValue) => {
            if (!childKey && !childValue) {
              ...
              shapeTypeDefinition.children[
                childKey
              ] = buildTypeAnnotationDeclarationTypes(childValue);
            }
          });
        ...
        case "UnionTypeAnnotation":
          ...
          for (let i = 0, j = annotation.types.length; i < j; i++) {
            const type = buildTypeAnnotationDeclarationTypes(
              annotation.types[i]
            );
            // keep only complex type
            if (Object.keys(type).length > 0) {
        ...
        case "ArrayTypeAnnotation":
          return {
            type: "object",
            children: {
              __ANY_KEY__: buildTypeAnnotationDeclarationTypes(
    ...
    function markPropTypesAsUsed(node, parentNames) {
        ...
        case "VariableDeclarator":
          for (let i = 0, j = node.id.properties.length; i < j; i++) {
            ...
            const thisDestructuring =
              ...
              (PROPS_REGEX.test(node.id.properties[i].key.name) ||
                PROPS_REGEX.test(node.id.properties[i].key.value)) &&
      ...
      const usedPropTypes = (
        (component && component.usedPropTypes) ||
        []
      ).slice();
      ...
      switch (type) {
        case "direct":
          ...
          const isDirectProp = DIRECT_PROPS_REGEX.test(
        ...
        case "destructuring":
          for (let k = 0, l = properties.length; k < l; k++) {
            ...
            while (
              ...
              !PROPS_REGEX.test(currentNode.property.name)
            ) {
      ...
      switch (propTypes && propTypes.type) {
        ...
        case "Identifier":
          ...
          for (let i = 0, j = variablesInScope.length; i < j; i++) {
            ...
            const defInScope =
              variablesInScope[i].defs[variablesInScope[i].defs.length - 1];
    ...
    function resolveSuperParameterPropsType(node) {
      ...
      try {
        ...
        propsParameterPosition = versionUtil.testFlowVersion(context, "0.53.0")
          ? 0
          : 1;
      } catch (e) {
    ...
    function markAnnotatedFunctionArgumentsAsDeclared(node) {
      if (
        !node.params ||
        !node.params.length ||
        !annotations.isAnnotatedFunctionPropsDeclaration(node, context)
      ) {
        return;
      }
    ...
    return {
      ...
      VariableDeclarator: function(node) {
        ...
        // let {props: {firstname}} = this
        const thisDestructuring =
          destructuring && node.init.type === "ThisExpression";
        // let {firstname} = props
        const directDestructuring =
          destructuring &&
          PROPS_REGEX.test(node.init.name) &&
      ...
      MethodDefinition: function(node) {
        if (
          !node.static ||
          node.kind !== "get" ||
          !isPropTypesDeclaration(node.key)
        ) {
          return;
        }
