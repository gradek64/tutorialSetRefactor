...
Components.prototype.add = function(node, confidence) {
  ...
  if (this._list[id]) {
    if (confidence === 0 || this._list[id].confidence === 0) {
      ...
      this._list[id].confidence = Math.max(
        this._list[id].confidence,
        confidence
      );
    }
...
Components.prototype.set = function(node, props) {
  ...
  if (this._list[id]) {
    ...
    copyUsedPropTypes =
      this._list[id].usedPropTypes && this._list[id].usedPropTypes.slice();
  }
  this._list[id] = util._extend(this._list[id], props);
  if (this._list[id] && props.usedPropTypes) {
    this._list[id].usedPropTypes = mergeUsedPropTypes(
      copyUsedPropTypes || [],
      props.usedPropTypes
    );
  }
...
Components.prototype.list = function() {
  ...
  for (const i in this._list) {
    ...
    if (component) {
      ...
      usedPropTypes[componentId] = (usedPropTypes[componentId] || []).concat(
        newUsedProps
      );
    }
  }
  ...
  for (const j in this._list) {
    ...
    list[j] = this._list[j];
    if (usedPropTypes[id]) {
      list[j].usedPropTypes = (list[j].usedPropTypes || []).concat(
        usedPropTypes[id]
      );
    }
  }
  ...
  const utils = {
    ...
    isReactCreateElement: function(node) {
      ...
      const calledDirectly =
        node && node.callee && node.callee.name === "createElement";
    ...
    isReturningJSX: function(ASTnode, strict) {
      ...
      const returnsReactCreateElement = this.isReactCreateElement(
        node[property]
      );

      return Boolean(
        returnsConditionalJSX || returnsJSX || returnsReactCreateElement
      );
    },
