  ...
  create(context) {
    ...
    function reportExpectedBraceError(node, bodyNode, name, suffix) {
      context.report({
        ...
        fix: fixer =>
          fixer.replaceText(bodyNode, `{${sourceCode.getText(bodyNode)}}`)
      });
    }
      ...
      context.report({
        ...
        fix(fixer) {
          ...
          const needsPrecedingSpace =
            ...
            sourceCode.getTokenBefore(bodyNode).range[1] ===
              bodyNode.range[0] &&
            !astUtils.canTokensBeAdjacent(
              "do",
              sourceCode.getFirstToken(bodyNode, { skip: 1 })
            );
    ...
    function prepareIfChecks(node) {
      ...
      do {
        preparedChecks.push(
          prepareCheck(node, node.consequent, "if", "condition")
        );
