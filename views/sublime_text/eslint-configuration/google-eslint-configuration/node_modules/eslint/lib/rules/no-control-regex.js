...
module.exports = {
  ...
  create(context) {
    ...
    const stringControlChar = /\\x[01][0-9a-f]/gi;
    const stringControlCharWithoutSlash = /x[01][0-9a-f]/gi;
    ...
    function getControlCharacters(regexStr) {
      ...
      if (subStrIndex > -1) {
        ...
        const possibleEscapeCharacters = regexStr
          .slice(0, subStrIndex)
          .match(consecutiveSlashesAtEnd);

        const hasControlChars =
          possibleEscapeCharacters === null ||
          !(possibleEscapeCharacters[0].length % 2);

        if (hasControlChars) {
          stringControlChars = regexStr
            .slice(subStrIndex, -1)
