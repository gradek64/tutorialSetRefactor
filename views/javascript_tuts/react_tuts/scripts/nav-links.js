...
Array.from(links).forEach((e,i)=>{
    if ( e.hasAttribute('partition')) {
        ...
        e.setAttribute('nonPartitionIndex','nonPartitionIndex'+nonPartitionIndex);
        nonPartitionIndex++;
    }
});
...
const callback = (i) => (evt) => {
    ...
    */
    ...
    sections.forEach((e)=>{e.classList.remove('activeSection');});
    ...
    if(currentPartition){
        ...
        partitions.forEach((e)=>{e.classList.remove('first');});
    ...
    if(currentNonPartitionIndex){
        ...
        if(secPartitions.length!==0){
            secPartitions.forEach((e)=>{e.classList.remove('first');});
...
Array.prototype.forEach.call(links,(e,i)=>{e.addEventListener('click',callback(i),false);});