...
*/
const ss_ts:(xType:string,yType:string)=> void = function(x:string,y:string) {
  console.log(x,y)
}

//ts format 
const ss_ts_es6:(xType:string,yType:string) => void = (x:string,y:string) => {
  console.log(x,y)
}
...
//ts type 
const addConst_ts:(x: number, y: number) => number = function(x: number, y: number){
  return x + y;
}
//ts type with es6
const addConst_ts_es6:(x: number, y: number) => number = (x: number, y: number) => {
  return x + y;
}

//ts contextualtype with es6 with only one parameter contextual
const addConst_ts_es6_cont:(xtype: number, ytypo: number) => number = (x, y: number) => {
  return x + y;
}
...
const addNonSpecificReturnConst_ts:(x: number, y: number) => number = (x: number, y: number) =>  x + y;

//ts format always contextual types are skiped in actual functon and kept in declarion
const addNonSpecificReturnConst_ts_cont:(x: number, y: number) => number = (x,y) =>  x + y;
...
const addConst_ts_es6_interface_skipped_return:AddConst_ts_es6_interface_skipped_return = (x, y: number) => {
  return x + y;
}
...
//typescript format;
const buildName_ts:(firstName: string, lastName?: string) => string =
...
const buildName_ts_cont:(firstName: string, lastName?: string) => string =
...
const buildName_ts_cont_interface:AddConst_ts_es6_interface_optional_param =
function name(firstName, lastName?)  {
  if (lastName)
      return firstName + " " + lastName;
  else
...
//es6 format
const buildNameDefaultConst = (firstName: string, lastName = "Smith"):string => firstName + " " + lastName;

//ts format type declaration dont take defaults cause they only define types 
const buildNameDefaultConst_ts:(firstName: string, lastName:string) => string 
    = (firstName: string, lastName = "Smith") => firstName + " " + lastName;

//ts contextual typing actual parameters types are skiped since defined in function declaration
const buildNameDefaultConst_ts_cont:(firstNameType: string, lastNameType:string) => string 
...
const buildName_ts_default_interface:AddConst_ts_es6_interface_optional =
function name(firstName, lastName='not known')  {
      return firstName + " " + lastName;
}
...
const rest_operator_ts:(firstNameType: string, ...restofName:string[]) => string 
= (firstName:string , ...restofName:[]) => firstName + " " + restofName.join(" ");

//ts contextual
const rest_operator_ts_cont:(firstNameType: string, ...restofName:string[]) => string 
= (firstName, ...restofName) => firstName + " " + restofName.join(" ");
...
const buildName_spread_operator_interface:AddConst_spread_operator = (firstName, ...otherNames:string[]) => {
...
const buildName_one_or_another_interface:AddConst_spread_operator = (data: string | number) => data
...
//ts format 
const setMeSingnature:(sign:()=>void)=>void = function(func:()=>void) {
  const result = func()
  return result; //allows to return even the return set to void
}
