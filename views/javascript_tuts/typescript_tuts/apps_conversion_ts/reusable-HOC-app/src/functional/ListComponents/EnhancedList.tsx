...
interface IHelperProps {
  ...
  */
const withTransformProps = (helperFunc:ICallbackFn):(props:IHelperProps)=>JSX.Element  => {

    //rember props are comming from Component as <ListWithTransformation list=[{}] />
    //according helperFunction=>WrapperComponent=>Component
  
  const withTransformPropsComp = (props:IHelperProps):JSX.Element => {
    /** ** props is a object defined in propTypes
    * */
    console.log('props of ListWithTransformation:', props);

    const transformedProps:{list:{name:string,side:string}[]} = helperFunc(props);
