...
const renderList =(filters)=> {
	...
	Array.from(navLinks).filter((e,i)=>{
		...
		const searchTextFilter = e.innerText.toLowerCase().trim().includes(filters.searchText);
		searchTextFilter ? navLinks[i].style.display = 'block' : navLinks[i].style.display = 'none';
	});
};
