  ...
  function getIcon(iconName) {
    switch (iconName) {
      ...
      case "play":
        ...
        path.setAttribute(
          "d",
          "M12 2c6.625 0 12 5.375 12 12s-5.375 12-12 12-12-5.375-12-12 5.375-12 12-12zM18 14.859c0.313-0.172 0.5-0.5 0.5-0.859s-0.187-0.688-0.5-0.859l-8.5-5c-0.297-0.187-0.688-0.187-1-0.016-0.313 0.187-0.5 0.516-0.5 0.875v10c0 0.359 0.187 0.688 0.5 0.875 0.156 0.078 0.328 0.125 0.5 0.125s0.344-0.047 0.5-0.141z"
        );
        svg.appendChild(path);
        return svg;
      case "book":
...
(function(win, doc, $) {
  ...
  $.fn.mozMenu = function(options) {
    ...
    return this.each(function() {
      ...
      var initialized;
      var $submenu = ($self.submenu =
        settings.submenu || $parent.find(".js-submenu"));
      ...
      $self.on("touchstart mouseenter focus", function(startEvent) {
        ...
        if (!initialized) {
          initialized = 1;
          var closeIcon = window.mdnIcons
            ? window.mdnIcons.getIcon("close")
            : "";
          var $closeButton = $(
            '<button type="button" class="submenu-close transparent">' +
              '<span class="offscreen">' +
              gettext("Close submenu") +
              "</span></button>"
          )
            .append(closeIcon)
            .prependTo($submenu);
        ...
        showTimeout = setTimeout(function() {
          $submenu
            ...
            .fadeIn($submenu.settings.fadeInSpeed);
  ...
  $.fn.mozKeyboardNav = function(options) {
    var settings = $.extend(
      { itemSelector: "a", onEnterKey: noop, alwaysCollectItems: false },
      options
    );
    var $selectedItem;
    return this.each(function() {
      ...
      $self.on("keydown", function(e) {
        ...
        if (code === 38 || code === 40) {
          ...
          var activeElementIndex =
            doc.activeElement && $items.index(doc.activeElement);
  ...
  $.fn.mozTogglers = function(options) {
    var settings = $.extend(
      { onOpen: noop, slideCallback: noop, duration: 200 },
      options
    );
    this.each(function() {
      var $self = $(this);
  ...
  $.fn.toggleMessage = function(options) {
    var settings = $.extend({ event: "click", toggleCallback: noop }, options);
    return this.each(function() {
      $(this).on(settings.event, function(e) {
        ...
        $self
          .attr("data-alternate-message", currentMessage)
  ...
  win.mdn.Notifier = (function() {
    ...
    function applyOptions($item, options) {
      ...
      $item.prepend(
        '<div class="notification-img"><i aria-hidden="true" class="' +
          icon +
          '"></i></div>'
      );
      if (options.url) {
    ...
    return {
      discover: function(parent) {
        var $notifications = $(parent || doc.body).find(
          ".notification:not([" + processedKey + "])"
        );
      ...
      growl: function(message, options) {
        ...
        if (!$tray) {
          $tray = $(
            '<div class="notification-tray" role="status" aria-live="polite"></div>'
          ).appendTo(doc.body);
        }
...
(function(win, doc, $) {
  ...
  var analytics = (mdn.analytics = {
    ...
    trackTiming: function(timingObject) {
      var data = {
        hitType: "timing",
        timingCategory: timingObject.category || "",
        timingVar: timingObject.timingVar || "",
        timingValue: timingObject.value ? Math.round(timingObject.value) : 0
      };
    ...
    trackOutboundLinks: function(target) {
      ...
      $(target).on("click", "a", function(e) {
        ...
        if (host && host !== location.hostname) {
          ...
          var data = { category: "Outbound Links", action: href };
          if (newTab) {
  ...
  (function() {
    ...
    $.each(
      [
        ...
        { selector: "body", method: "prependTo" }
      ],
      function() {
    ...
    $.each(mdn.notifications || [], function() {
      ...
      $("<div />")
        ...
        .html(messageHTML)
        [insertLocation.method](insertLocation.selector);
    });
        ...
        if (matchCurrent && !matchStored) {
          ...
          $doc.on("mdn:login", function() {
            mdn.Notifier.growl(
              "You can now use " +
                matchStored +
                " to sign in to this MDN profile.",
...
(function(win, doc, $) {
  ...
  (function() {
    ...
    $("<link />")
      .attr({ href: compatCSS, type: "text/css", rel: "stylesheet" })
      .on("load", function() {
        $.ajax({ url: compatJS, dataType: "script", cache: true }).then(
          function() {
            ...
            $.ajax({
              ...
              console.error(
                "Failed to load BCD survey JS: " + textStatus + ": ",
                errorThrown
              );
            });
          }
...
(function(win, doc, $) {
  ...
  function storeLocaleChange(code, name) {
    if (!isLocalePreference(code)) {
      sessionStorage.setItem(
        sessionStorageKey,
        JSON.stringify({ code: code, name: name })
      );
    }
  }
  function getCookie(name) {
  ...
  function trackGAEvent(action, locale) {
    if (win.ga) {
      var data = { category: "Remember Language", action: action };
      if (locale) {
        data.label = locale;
      }
      mdn.analytics.trackEvent(data);
  ...
  if (win.sessionStorage && win.mdn.features.localStorage && !neverShowNotice) {
    ...
    if (changedLocaleTo) {
      ...
      var text = gettext(
        "You are now viewing this site in %(localeName)s." +
          " Do you always want to view this site in %(localeName)s?"
      );
      var message = interpolate(text, { localeName: locale.name }, true);
      var button =
        '<button id="%s" type="button" data-locale="' +
        locale.code +
        '">%s</button> ';
      var yesButton = interpolate(button, [
        "locale-permanent-yes",
        gettext("Yes")
      ]);
      var noButton = interpolate(button, [
        "locale-permanent-no",
        gettext("No")
      ]);
      var neverButton = interpolate(button, [
        "locale-permanent-never",
        gettext("Never")
      ]);
      var html = message + "<br>" + yesButton + noButton + neverButton;
      var notification = mdn.Notifier.growl(html, {
        closable: true,
        duration: 0
      });
      ...
      $("#locale-permanent-yes").on("click", function() {
        ...
        $.post("/i18n/setlang/", { language: locale }).success(function() {
          notification.close();
        });
