(function(doc) {
  ...
  if (!newsletterForm) {
    ...
    if (mdn.features.localStorage) {
      isHidden =
        localStorage.getItem("newsletterHide") === "true" ? true : false;
    }
  ...
  function newsletter() {
    ...
    function newsletterSubscribe(event) {
      ...
      var privacy = doc.querySelector('input[name="privacy"]:checked')
        ? "&privacy=true"
        : "";
      var params =
        "email=" +
        ...
        fmt +
        "&source_url=" +
        encodeURIComponent(doc.location.href);
    ...
    newsletterEmailInput.addEventListener(
      "focus",
      function() {
